...
const mapStarships = (input) => {
  ...
  const mapStarships = input.map(MapFunc);
  return mapStarships;
};
function MapFunc(item) {
  if (item.cost_in_credits === "unknown")
    ...
    return `${item.name}, manufactured by ${
      item.manufacturer
    } - cost: ${numberWithCommas(item.cost_in_credits)} credits`;
}
const filterStarships = (input) => {
  ...
  const ships = input.filter((item) => item.crew > 1 && item.passengers < 10);
  return ships;
};

const reduceStarships = (input) => {
  // Return the cost to purchase all ships in the input array
  const arr = input.reduce((total, item) => {
    if (item.cost_in_credits !== "unknown") {
      total = total + parseInt(item.cost_in_credits);
    }
    return total;
  }, 0);
  return `The cost of all starships is ${numberWithCommas(arr)} credits`;
};
